...
import { isGoogleDriveStorageEnabled } from "@/lib/storage";
  ...
  try {
    if (!isGoogleDriveStorageEnabled()) {
      return NextResponse.json(
        { success: false, error: "GOOGLE_DRIVE_STORAGE_DISABLED" },
        { status: 409 }
      );
    }
    ...
    if (!isAllowedDriveUploadUrl(uploadUrl)) {
      return NextResponse.json({ success: false, error: "INVALID_DRIVE_UPLOAD_URL" }, { status: 400 });
    }
    if (

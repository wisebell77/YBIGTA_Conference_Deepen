...
export default function GraphWorkspace({ projectId }: { projectId: string }) {
  ...
  const loadGoogleAuthStatus = useCallback(async () => {
    ...
    setGoogleAuth({
      ...
      user: json.user,
      storageBackend: json.storageBackend ?? "local"
    });
  }, []);
  ...
  useEffect(() => {
    ...
    loadGoogleAuthStatus().catch(() =>
      setGoogleAuth({ connected: false, user: null, storageBackend: "local" })
    );
  ...
  const uploadPdf = async (file: File) => {
    ...
    setIsUploading(true);
    try {
      if (googleAuth?.connected && googleAuth.storageBackend === "google_drive") {
  ...
  const logoutGoogleDrive = async () => {
    ...
    setGoogleAuth({
      connected: false,
      user: null,
      storageBackend: googleAuth?.storageBackend ?? "local"
    });

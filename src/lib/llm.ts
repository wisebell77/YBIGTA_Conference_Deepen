...
- Do not invent authors or year if they are not present.`;
  ...
  try {
    ...
    if ((error as Error).message === "OPENAI_API_KEY_MISSING") {
    ...
    if ((error as Error).message === "OPENAI_API_KEY_MISSING") {
      return {
        edges: params.candidates.slice(0, params.edgeLimit).map((paper) => ({
          ...
          longDescription:
            "This relationship was generated by the local fallback because OPENAI_API_KEY is not configured. In production, the LLM evaluates conceptual and methodological relationships between the new paper and selected candidates.",
